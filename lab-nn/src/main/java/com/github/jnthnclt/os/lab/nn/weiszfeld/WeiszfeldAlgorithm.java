...
public class WeiszfeldAlgorithm {
    ...
    static public Output process(Input input) {
        ...
        for (Map.Entry<Point, Double> entry : map.entrySet()) {
            WeightedPoint wPoint = new WeightedPoint(entry.getKey(), entry.getValue());
            aPoints.add(wPoint);
        }
    ...
    static private Point operatorT(Point x, List<WeightedPoint> aPoints, int dimension) {
    ...
    static private Point operatorS(Point aj, double wj, Point rj
    ...
    static private Point R(Point aj, List<WeightedPoint> aPoints, int dimension) {
        Point result = new Point(dimension);
    ...
    static private double operatorL(Point aj, List<WeightedPoint> aPoints) {
        double res = 0;
        for (WeightedPoint ai: aPoints) {
     ...
     * @return
     */
    static private double evaluateF(Point x, List<WeightedPoint> aPoints) {
        double res = 0;
        for (WeightedPoint ai: aPoints) {
